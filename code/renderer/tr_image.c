...
void LoadDDS( const char *name, byte **pic, int *width, int *height );
...
void R_ImageList_f( void ) {
	...
	for ( i = 0 ; i < tr.numImages ; i++ ) {
        ...
        // @pjb: todo: all this reports incorrectly for non-pow2 textures
		texels += image->width*image->height;
		RI_Printf (PRINT_ALL,  "%4i: %4i %4i  %s   ",
			i, image->width, image->height, yesno[image->mipmap] );
		switch ( image->format ) {
		case IMAGEFORMAT_I:
			RI_Printf( PRINT_ALL, "I    " );
			break;
		case IMAGEFORMAT_IA:
			RI_Printf( PRINT_ALL, "IA   " );
			break;
		case IMAGEFORMAT_RGB:
			RI_Printf( PRINT_ALL, "RGB  " );
			break;
		case IMAGEFORMAT_RGBA:
			RI_Printf( PRINT_ALL, "RGBA " );
			break;
		case IMAGEFORMAT_RGBA8:
			RI_Printf( PRINT_ALL, "RGBA8" );
			break;
		case IMAGEFORMAT_RGB8:
			RI_Printf( PRINT_ALL, "RGB8" );
			break;
		case IMAGEFORMAT_S3TC:
			RI_Printf( PRINT_ALL, "S3TC " );
			break;
		case IMAGEFORMAT_RGBA4:
			RI_Printf( PRINT_ALL, "RGBA4" );
			break;
		case IMAGEFORMAT_RGB5:
			RI_Printf( PRINT_ALL, "RGB5 " );
			break;
		default:
			RI_Printf( PRINT_ALL, "???? " );
		}
		...
		case WRAPMODE_REPEAT:
			RI_Printf( PRINT_ALL, "rept " );
			break;
		case WRAPMODE_CLAMP:
			RI_Printf( PRINT_ALL, "clmp " );
			break;
		default:
			RI_Printf( PRINT_ALL, "%4i ", image->wrapClampMode );
			break;
		}
	...
	RI_Printf (PRINT_ALL, " ---------\n");
	RI_Printf (PRINT_ALL, " %i total texels (not including mipmaps)\n", texels);
	RI_Printf (PRINT_ALL, " %i total images\n\n", tr.numImages );
}
...
image_t *R_CreateImage( const char *name, const byte *pic, int width, int height, 
					   qboolean mipmap, qboolean allowPicmip, wrapClampMode_t wrapClampMode ) {
	image_t		*image;
...
static void LoadPCX ( const char *filename, byte **pic, byte **palette, int *width, int *height)
{
	...
	*palette = NULL;
	...
	if (pcx->manufacturer != 0x0a
		...
		|| ymax >= 1024)
	{
		RI_Printf (PRINT_ALL, "Bad pcx file %s (%i x %i) (%i x %i)\n", filename, xmax+1, ymax+1, pcx->xmax, pcx->ymax);
		return;
	}

	out = RI_Malloc ( (ymax+1) * (xmax+1) );
...
// FIXME: use bytes_per_line here?
	...
	if ( raw - (byte *)pcx > len)
	{
		RI_Printf (PRINT_DEVELOPER, "PCX file %s was malformed", filename);
		RI_Free (*pic);
		*pic = NULL;
	}
...
static void LoadTGA ( const char *name, byte **pic, int *width, int *height)
{
	...
	*pic = NULL;
	...
	targa_rgba = RI_Malloc (numPixels*4);
	*pic = targa_rgba;
  ...
  if (targa_header.attributes & 0x20) {
    RI_Printf( PRINT_WARNING, "WARNING: '%s' TGA file header declares top-down image, ignoring\n", name);
  }
...
static void LoadJPG( const char *filename, unsigned char **pic, int *width, int *height ) {
  ...
  out = RI_Malloc(cinfo.output_width*cinfo.output_height*cinfo.output_components);
...
void SaveJPG(char * filename, int quality, int image_width, int image_height, unsigned char *image_buffer) {
  ...
  FS_WriteFile( filename, out, (int) hackSize );
...
void R_LoadImage( const char *name, byte **pic, int *width, int *height ) {
	...
	*height = 0;
    ...
    // @pjb: Intercept and see if we can find a DDS for this image
    {
	...
	if ( !Q_stricmp( name+len-4, ".tga" ) ) {
        LoadTGA( name, pic, width, height );            // try tga second
        if (!*pic) {                                    //
...
==============
*/
image_t	*R_FindImageFile( const char *name, qboolean mipmap, qboolean allowPicmip, wrapClampMode_t wrapClampMode ) {
	image_t	*image;
	int		width, height;
	byte	*pic;
	long	hash;
		...
		if ( !strcmp( name, image->imgName ) ) {
			...
			if ( strcmp( name, "*white" ) ) {
				if ( image->mipmap != mipmap ) {
					RI_Printf( PRINT_DEVELOPER, "WARNING: reused image %s with mixed mipmap parm\n", name );
				}
				if ( image->allowPicmip != allowPicmip ) {
					RI_Printf( PRINT_DEVELOPER, "WARNING: reused image %s with mixed allowPicmip parm\n", name );
				}
				if ( image->wrapClampMode != wrapClampMode ) {
					RI_Printf( PRINT_ALL, "WARNING: reused image %s with mixed wrapClampMode parm\n", name );
				}
			}
			return image;
		}
	...
	image = R_CreateImage( ( char * ) name, pic, width, height, mipmap, allowPicmip, wrapClampMode );
	RI_Free( pic );
	return image;
}
...
static void R_CreateDlightImage( void ) {
	...
	tr.dlightImage = R_CreateImage("*dlight", (byte *)data, DLIGHT_SIZE, DLIGHT_SIZE, qfalse, qfalse, WRAPMODE_CLAMP );
}
...
static void R_CreateDefaultImage( void ) {
	...
	tr.defaultImage = R_CreateImage("*default", (byte *)data, DEFAULT_SIZE, DEFAULT_SIZE, qtrue, qfalse, WRAPMODE_REPEAT );
}
...
void R_CreateBuiltinImages( void ) {
	...
	tr.whiteImage = R_CreateImage("*white", (byte *)data, 8, 8, qfalse, qfalse, WRAPMODE_REPEAT );
	...
	tr.identityLightImage = R_CreateImage("*identityLight", (byte *)data, 8, 8, qfalse, qfalse, WRAPMODE_REPEAT );


	for(x=0;x<32;x++) {
		// scratchimage is usually used for cinematic drawing
		tr.scratchImage[x] = R_CreateImage("*scratch", (byte *)data, DEFAULT_SIZE, DEFAULT_SIZE, qfalse, qtrue, WRAPMODE_CLAMP );
	}
...
void R_SetColorMappings( void ) {
	...
	GFX_SetGamma( r_gamma->value, r_intensity->value );
}
...
qhandle_t RE_RegisterSkin( const char *name ) {
	...
	if ( tr.numSkins == MAX_SKINS ) {
		RI_Printf( PRINT_WARNING, "WARNING: RE_RegisterSkin( '%s' ) MAX_SKINS hit\n", name );
		return 0;
	}
	tr.numSkins++;
...
void	R_SkinList_f( void ) {
	...
	for ( i = 0 ; i < tr.numSkins ; i++ ) {
		...
		RI_Printf( PRINT_ALL, "%3i:%s\n", i, skin->name );
		for ( j = 0 ; j < skin->numSurfaces ; j++ ) {
			RI_Printf( PRINT_ALL, "       %s = %s\n", 
