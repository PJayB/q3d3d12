...
namespace QD3D12
{
    ...
    void PipelineState::LoadCache()
    {
        ...
        for (auto hash : hashes)
        {
            ...
            if (m_db->LoadCachedBlob(hash, &cachedPSO))
            {
                ...
                ConvertCachedPSOBlobToState(cachedPSO, m_pipelineDesc.pRootSignature, &state);

                ID3D12PipelineState* pso = nullptr;
                HRESULT hr = Device::Get()->DeserializeGraphicsPipelineStateX(
                    &state,
        ...
        BOOL bLateCache)
	{
        ...
        if (m_db->LoadCachedBlob(hash, &cachedPSO))
        {
            ...
            D3D12XBOX_DESERIALIZE_GRAPHICS_PIPELINE_STATE state;
            ConvertCachedPSOBlobToState(cachedPSO, m_pipelineDesc.pRootSignature, &state);
            COM_ERROR_TRAP(Device::Get()->DeserializeGraphicsPipelineStateX(
                &state,
                __uuidof(ID3D12PipelineState),
                (void**) &pState));
#else
