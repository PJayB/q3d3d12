    ...
    ID3D11InputLayout** layout )
{
    ...
    *vshader = LoadVertexShader( shaderName, &vsByteCode );
    ...
    HRESULT hr = g_pDevice->CreateInputLayout(
        ...
        layout );
    if ( FAILED( hr ) ) {
        RI_Error( ERR_FATAL, "Failed to create input layout: 0x%08X", hr );
    }
...
void InitQuadRenderData( d3dQuadRenderData_t* qrd ) 
{
    ...
    qrd->indexBuffer = QD3D11::CreateImmutableBuffer( 
        g_pDevice, 
        D3D11_BIND_INDEX_BUFFER, 
        indices,
        sizeof( indices ) );
    if ( !qrd->indexBuffer ) {
        RI_Error( ERR_FATAL, "Could not create FSQ index buffer.\n" );
    }

    qrd->vertexBuffer = QD3D11::CreateDynamicBuffer<d3dQuadRenderVertex_t>(
        g_pDevice, 
        D3D11_BIND_VERTEX_BUFFER,
        4);
    if ( !qrd->vertexBuffer ) {
        RI_Error( ERR_FATAL, "Could not create FSQ vertex buffer.\n" );
    }
}
...
void InitSkyBoxRenderData( d3dSkyBoxRenderData_t* rd ) 
{
    ...
    rd->vertexBuffer = QD3D11::CreateImmutableBuffer(
        g_pDevice, 
        D3D11_BIND_VERTEX_BUFFER,
        skyboxVertexData,
        sizeof(skyboxVertexData) );
    if ( !rd->vertexBuffer ) {
        RI_Error( ERR_FATAL, "Could not create SkyBox vertex buffer.\n" );
    }

    rd->vsConstantBuffer = QD3D11::CreateDynamicBuffer<d3dSkyBoxVSConstantBuffer_t>( 
        g_pDevice, 
        D3D11_BIND_CONSTANT_BUFFER );
    if ( !rd->vsConstantBuffer ) {
        RI_Error( ERR_FATAL, "Could not create SkyBox VS constant buffer.\n" );
    }

    rd->psConstantBuffer = QD3D11::CreateDynamicBuffer<d3dSkyBoxPSConstantBuffer_t>( 
        g_pDevice, 
        D3D11_BIND_CONSTANT_BUFFER );
    if ( !rd->psConstantBuffer ) {
        RI_Error( ERR_FATAL, "Could not create SkyBox PS constant buffer.\n" );
    }
}
...
void InitViewRenderData( d3dViewRenderData_t* vrd )
{
    ...
    vrd->vsConstantBuffer = QD3D11::CreateDynamicBuffer<d3dViewVSConstantBuffer_t>( 
        g_pDevice, 
        D3D11_BIND_CONSTANT_BUFFER );
    if ( !vrd->vsConstantBuffer ) {
        RI_Error( ERR_FATAL, "Could not create view constant buffer.\n" );
    }

    vrd->psConstantBuffer = QD3D11::CreateDynamicBuffer<d3dViewPSConstantBuffer_t>( 
        g_pDevice, 
        D3D11_BIND_CONSTANT_BUFFER );
...
static ID3D11DepthStencilState* CreateDepthStencilStateFromMask( unsigned long mask )
{
    ...
    if ( !state ) {
        RI_Error( ERR_FATAL, "Failed to create DepthStencilState of mask %x\n", mask );
    }
...
static d3dCircularBuffer_t CreateTessIndexBuffer()
{
    ...
    if ( !buffer ) {
        RI_Error( ERR_FATAL, "Could not create tess index buffer.\n" );
    }
...
static d3dCircularBuffer_t CreateTessVertexBuffer( size_t size )
{
	...
	g_pDevice->CreateBuffer(&bd, NULL, &buffer);
    if ( !buffer ) {
        RI_Error( ERR_FATAL, "Could not create tess vertex buffer.\n" );
    }
...
void InitTessBuffers( d3dTessBuffers_t* tessbuf )
{
    Com_Memset( tessbuf, 0, sizeof( *tessbuf ) );

    // Index buffer
    tessbuf->indexes = CreateTessIndexBuffer();

    // Vertex buffer
    tessbuf->xyz = CreateTessVertexBuffer( sizeof(vec4_t) );
    ...
    {
        d3dTessStageBuffers_t* stageBufs = &tessbuf->stages[stage];
    ...
    {
        d3dTessLightProjBuffers_t* buffers = &tessbuf->dlights[l];
    ...
    //
    tessbuf->fog.colors = CreateTessVertexBuffer( sizeof(color4ub_t) );
    tessbuf->fog.texCoords = CreateTessVertexBuffer( sizeof(vec2_t) );
}

void DestroyTessBuffers( d3dTessBuffers_t* tessbuf )
{
    SAFE_RELEASE( tessbuf->indexes.buffer );
    SAFE_RELEASE( tessbuf->xyz.buffer );

    for ( int stage = 0; stage < MAX_SHADER_STAGES; ++stage )
    {
        SAFE_RELEASE( tessbuf->stages[stage].colors.buffer );
        for ( int tex = 0; tex < NUM_TEXTURE_BUNDLES; ++tex )
            SAFE_RELEASE( tessbuf->stages[stage].texCoords[tex].buffer );
    }

    for ( int l = 0; l < MAX_DLIGHTS; ++l )
    {
        SAFE_RELEASE( tessbuf->dlights[l].indexes.buffer );
        SAFE_RELEASE( tessbuf->dlights[l].colors.buffer );
        SAFE_RELEASE( tessbuf->dlights[l].texCoords.buffer );
    }

    SAFE_RELEASE( tessbuf->fog.colors.buffer );
    SAFE_RELEASE( tessbuf->fog.texCoords.buffer );
