...
void UpdateDirtyViewVS()
{
    ...
    d3dViewVSConstantBuffer_t* cb = QD3D11::MapDynamicBuffer<d3dViewVSConstantBuffer_t>( g_pImmediateContext, g_DrawState.viewRenderData.vsConstantBuffer );
...
{
    // Upload the constants
    d3dViewPSConstantBuffer_t* cb = QD3D11::MapDynamicBuffer<d3dViewPSConstantBuffer_t>( g_pImmediateContext, g_DrawState.viewRenderData.psConstantBuffer );
...
static void SetTessVertexBuffer( const d3dTessBuffers_t* tessbuf )
{
    UINT stride = sizeof(vec4_t);
    UINT offset = tessbuf->xyz.currentOffset;
    g_pImmediateContext->IASetVertexBuffers( 0, 1, &tessbuf->xyz.buffer, &stride, &offset );
}

static void SetTessVertexBuffersST( const d3dTessStageBuffers_t* stage )
{
    ...
    g_pImmediateContext->IASetVertexBuffers( 2, 2, vbufs, strides, offsets );
}

static void SetTessVertexBuffersMT( const d3dTessStageBuffers_t* stage )
{
    ...
    g_pImmediateContext->IASetVertexBuffers( 2, 3, vbufs, strides, offsets );
}
    ...
    const float* in_color )
{
    ...
    //
    d3dQuadRenderVertex_t* vb = QD3D11::MapDynamicBuffer<d3dQuadRenderVertex_t>( 
        g_pImmediateContext, 
        qrd->vertexBuffer );
    ...
    // Draw
    //
    UINT stride = sizeof(d3dQuadRenderVertex_t);
    UINT offset = 0;
    ...
    const float* colorTint )
{
    ...
    //
    d3dSkyBoxVSConstantBuffer_t* vscb = QD3D11::MapDynamicBuffer<d3dSkyBoxVSConstantBuffer_t>( 
        g_pImmediateContext, 
    ...
    //
    d3dSkyBoxPSConstantBuffer_t* pscb = QD3D11::MapDynamicBuffer<d3dSkyBoxPSConstantBuffer_t>( 
        g_pImmediateContext, 
...
void D3D11API_ShadowSilhouette( const float* edges, int edgeCount )
{

}

void D3D11API_ShadowFinish( void )
{
...
void D3D11API_DrawBeam( const image_t* image, const float* color, const vec3_t startPoints[], const vec3_t endPoints[], int segs )
{
...
static void TessDrawTextured( const shaderCommands_t* input, int stage )
{
	...
	if ( pStage->bundle[0].vertexLightmap && r_lightmap->integer ) {
    ...
    SetTessVertexBuffersST( &buffers->stages[stage] );
...
static void TessDrawMultitextured( const shaderCommands_t* input, int stage )
{
    ...
    SetTessVertexBuffersMT( &buffers->stages[stage] );
...
static void TessProjectDynamicLights( const shaderCommands_t *input )
{
    ...
    for ( int l = 0 ; l < input->dlightCount ; l++ )
    {
		...
		if ( dlInfo->additive ) {
			D3D11API_SetState( GLS_SRCBLEND_ONE | GLS_DSTBLEND_ONE | GLS_DEPTHFUNC_EQUAL );
		}
		else {
			D3D11API_SetState( GLS_SRCBLEND_DST_COLOR | GLS_DSTBLEND_ONE | GLS_DEPTHFUNC_EQUAL );
		}
...
static void TessDrawFog( const shaderCommands_t* input )
{
	if ( input->shader->fogPass == FP_EQUAL ) {
		D3D11API_SetState( GLS_SRCBLEND_SRC_ALPHA | GLS_DSTBLEND_ONE_MINUS_SRC_ALPHA | GLS_DEPTHFUNC_EQUAL );
	} else {
		D3D11API_SetState( GLS_SRCBLEND_SRC_ALPHA | GLS_DSTBLEND_ONE_MINUS_SRC_ALPHA );
	}
...
static void IterateStagesGeneric( const shaderCommands_t *input )
{
    for ( int stage = 0; stage < MAX_SHADER_STAGES; stage++ )
	{
        ...
        D3D11API_SetState( pStage->stateBits );
...
D3D_PUBLIC void D3D11API_DrawStageDepth(const shaderCommands_t *input)
{
	PROFILE_FUNC();
	assert(RB_IsDepthPass());

    // TODO: optimizations here
    D3D11API_DrawStageGeneric( input );
}

D3D_PUBLIC void D3D11API_DrawStageSky(const shaderCommands_t *input)
{
	PROFILE_FUNC();
