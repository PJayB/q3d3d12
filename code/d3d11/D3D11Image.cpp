    ...
    qboolean isLightmap )
{
    ...
    void* lightscaledCopy = Hunk_AllocateTempMemory( (int) imageSizeBytes );
    ...
    D3D11_SUBRESOURCE_DATA* subres = (D3D11_SUBRESOURCE_DATA*) Hunk_AllocateTempMemory( (int) sizeof( D3D11_SUBRESOURCE_DATA ) * mipLevels );
    for ( int mip = 0; mip < mipLevels; ++mip )
    {
    	...
    	subres[mip].pSysMem = Hunk_AllocateTempMemory( mipSize );
    ...
    for ( int mip = mipLevels - 1; mip >= 0; --mip )
        Hunk_FreeTempMemory( (void*) subres[mip].pSysMem );
    
    Hunk_FreeTempMemory( subres );
    Hunk_FreeTempMemory( lightscaledCopy );

    // Create a shader resource view
    d3dImage->pSRV = QD3D11::CreateTexture2DShaderResourceView( 
        g_pDevice,
        d3dImage->pTexture,
...
D3D_PUBLIC void D3D11API_CreateImage( const image_t* image, const byte *pic, qboolean isLightmap )
{
...
D3D_PUBLIC void D3D11API_DeleteImage( const image_t* image )
{
    d3dImage_t* d3dImage = &s_d3dImages[image->index];
...
D3D_PUBLIC void D3D11API_UpdateCinematic( const image_t* image, const byte* pic, int cols, int rows, qboolean dirty )
{
    if ( cols != image->width || rows != image->height ) {
        D3D11API_DeleteImage( image );
        ...
        if (dirty) {
            ...
            // Check it's dynamic
            if ( !d3dImage->dynamic )
                RI_Error( ERR_FATAL, "Trying to upload cinematic texture to a non-dynamic image.\n" );
...
D3D_PUBLIC imageFormat_t D3D11API_GetImageFormat( const image_t* image )
{
    // @pjb: hack: all images are RGBA8 for now
    return IMAGEFORMAT_RGBA;
}

D3D_PUBLIC int D3D11API_SumOfUsedImages( void )
{
	int	total;
	int i;
...
const d3dImage_t* GetImageRenderInfo( const image_t* image )
{
    ...
    if ( !d3dImage->pTexture ) {
        RI_Error( ERR_FATAL, "Illegal image index.\n" );
    }
