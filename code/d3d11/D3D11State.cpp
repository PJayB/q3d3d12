...
//----------------------------------------------------------------------------
void QD3D11::CreateBuffers()
{
    g_BufferState.backBufferView = QD3D11::CreateBackBufferView(g_pSwapChain, g_pDevice, &g_BufferState.backBufferDesc);
    ASSERT(g_BufferState.backBufferView);

    g_BufferState.depthBufferView = QD3D11::CreateDepthBufferView(
        g_pDevice,
...
// Set the culling mode depending on whether it's a mirror or not
//----------------------------------------------------------------------------
void D3D11API_SetState( unsigned long stateBits )
{
...
void InitDrawState()
{
    ...
    // Create D3D objects
    QD3D11::DestroyBuffers();
    QD3D11::CreateBuffers();
    InitImages();
    InitShaders();
    ...
    D3D11API_SetViewport( 0, 0, g_BufferState.backBufferDesc.Width, g_BufferState.backBufferDesc.Height );
